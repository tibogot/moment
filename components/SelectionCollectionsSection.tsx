 ...
 * above a product carousel — drag/swipe with inertia (royal-cashmere pattern)
 * plus arrow buttons; cards stay clickable.
 */
export function SelectionCollectionsSection({
  ...
  const visibleTabs = tabs.filter((tab) => byHandle.has(tab.handle));

  const [activeHandle, setActiveHandle] = useState(() => {
    if (byHandle.has(DEFAULT_HANDLE)) return DEFAULT_HANDLE;
    return visibleTabs[0]?.handle ?? null;
  });
  ...
  const active =
    (activeHandle && byHandle.get(activeHandle)) ||
    (visibleTabs[0] && byHandle.get(visibleTabs[0].handle)) ||
    null;
  ...
  useGSAP(
    ...
    {
      scope: containerRef,
      dependencies: [active?.handle, productCount],
    },
  );
    ...
    <GridSection className={cn("pt-[10svh] pb-[14svh]", className)}>
      <div className="col-start-2 col-end-5 flex flex-col items-start gap-4 px-(--grid-gutter) pb-[5svh] md:col-end-9 md:flex-row md:items-end md:justify-between">
        <ul
          ...
          role="tablist"
          aria-label={visibleTabs.map((tab) => tab.label).join(", ")}
        >
          {visibleTabs.map((tab) => {
            const selected = tab.handle === active.handle;
            return (
              <li key={tab.handle} role="presentation">
                <button
                  type="button"
                  role="tab"
                  aria-selected={selected}
                  aria-controls="selection-products-carousel"
                  id={`selection-tab-${tab.handle}`}
                  onClick={() => setActiveHandle(tab.handle)}
                  className="animated-underline font-owners-medium cursor-pointer text-[12px] uppercase tracking-wide"
                >
                  {tab.label}
                </button>
              </li>
        ...
        <div className="flex items-center gap-3">
          ...
          <Link
            href={routes.collection(active.handle)}
      ...
      <div
        ref={containerRef}
        id="selection-products-carousel"
        role="tabpanel"
        aria-labelledby={`selection-tab-${active.handle}`}
        className="col-start-2 col-end-5 min-w-0 md:col-end-9"
      >
        {active.products.length === 0 ? (
          <p className="font-archivo-light px-(--grid-gutter) text-[15px]">—</p>
        ) : (
          <div
            ref={viewportRef}
          ...
          >
            ...
            >
              {active.products.map((product) => (
                <li
                  key={product.id}
                  className="product-card w-[calc(var(--selection-carousel-width)/2)] shrink-0 border-b border-l border-sky transition-colors duration-500 md:w-[calc(var(--selection-carousel-width)/3)]"
                >
                  <ProductCard
                    product={product}
